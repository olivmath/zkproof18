...
export const loadQRCodeLibrary = async (): Promise<boolean> => {
  ...
  try {
    ...
    return new Promise((resolve) => {
      ...
      script.onerror = () => {
        console.log('QR Code library failed to load, using fallback');
        resolve(false);
      };
...
const generateFallbackQR = (canvas: HTMLCanvasElement, text: string, size: number) => {
  ...
  // Top-right marker
  drawCornerMarker(ctx, canvas.width - markerMargin - markerSize * cellSize, markerMargin, markerSize, cellSize);
  
  // Bottom-left marker
  drawCornerMarker(ctx, markerMargin, canvas.height - markerMargin - markerSize * cellSize, markerSize, cellSize);
};

const drawCornerMarker = (ctx: CanvasRenderingContext2D, x: number, y: number, size: number, cellSize: number) => {
  // Outer square
  ctx.fillStyle = '#000000';
